...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    const checkValidCustomer = await this.customersRepository.findById(
      customer_id,
    );
    ...
    const findNotAvailableProducts = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (findNotAvailableProducts.length) {
      throw new AppError(
        `Not enough items for product ${findNotAvailableProducts[0].id}`,
      );
    }
